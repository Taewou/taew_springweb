...
public class LetterDao {
	...
	static final String SENDER_LETTERS = "select letterId,title,receiverId,receiverName,cdate from letter where senderId=?";
	
	static final String RECEIVER_LETTERS = "select letterId,title,senderId,senderName,cdate from letter where receiverId=?";
	...
	public List<Letter> listLettersOfSender(String senderId) {
		return jdbcTemplate.query(SENDER_LETTERS, letterRowMapper,
				senderId);
	}
	...
	public List<Letter> listLettersOfReceiver(String receiverId) {
		return jdbcTemplate.query(RECEIVER_LETTERS, letterRowMapper,
				receiverId);
	}
